...
class HostProcessesTest {
    ...
    fun labelsKnownRuntimeCommands() {
        assertEquals("Pi", hostProcessLabel("node", "/usr/local/bin/node /usr/local/lib/pirt/pirt-control-bridge.mjs"))
        ...
        assertEquals("PIRT", hostProcessLabel("om.example.pirt", "com.example.pirt"))
    ...
    fun onlyWorkspaceProcessesAreStoppable() {
        ...
        val app = HostProcess(302, 1, "om.example.pirt", "com.example.pirt", HostProcessKind.APP)
    ...
    fun buildsNestedProcessForest() {
        val processes = listOf(
            HostProcess(100, 1, "om.example.pirt", "com.example.pirt", HostProcessKind.APP),
