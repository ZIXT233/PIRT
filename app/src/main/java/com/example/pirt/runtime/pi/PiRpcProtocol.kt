...
enum class TurnState { IDLE, QUEUED, GENERATING, RUNNING_TOOL, STOPPING, COMPLETED, FAILED }
...
sealed interface PiStreamEvent {
    ...
    data class ToolUpdated(val id: String, val output: String) : PiStreamEvent
    data class ToolEnded(val id: String, val name: String, val output: String, val failed: Boolean) : PiStreamEvent
...
sealed class PiRequest<T>(
    ...
    data class Prompt(val message: String, val images: List<PiImage>) : PiRequest<Unit>("prompt", 30_000L) {
    ...
    data class SetModel(val provider: String, val modelId: String) : PiRequest<PiModel>("set_model") {
        override fun payload() = base().put("provider", provider).put("modelId", modelId)
        override fun decode(data: JSONObject?): PiModel = checkNotNull(data).toModel()
    }
...
internal fun parseStreamEvent(json: JSONObject): PiStreamEvent? = when (json.optString("type")) {
    ...
    "compaction_start" -> PiStreamEvent.Phase(TurnState.GENERATING)
    "auto_retry_start" -> PiStreamEvent.Phase(TurnState.QUEUED)
    ...
    "tool_execution_update" -> PiStreamEvent.ToolUpdated(
        json.optString("toolCallId"),
        resultText(json.optJSONObject("partialResult")),
    )
    "tool_execution_end" -> PiStreamEvent.ToolEnded(
        json.optString("toolCallId"),
        json.optString("toolName"),
        resultText(json.optJSONObject("result")),
    ...
    "agent_end", "turn_end", "compaction_end", "auto_retry_end",
