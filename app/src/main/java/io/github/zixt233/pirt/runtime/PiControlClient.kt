...
import io.github.zixt233.pirt.runtime.pi.PiFailure
import io.github.zixt233.pirt.runtime.pi.PiRequest
import io.github.zixt233.pirt.runtime.pi.PiRequestException
import io.github.zixt233.pirt.runtime.pi.PiStreamEvent
import io.github.zixt233.pirt.runtime.pi.parseStreamEvent
...
sealed interface PiAuthEvent {
    ...
    data class Error(val command: String?, val message: String, val requestId: String? = null) : PiAuthEvent
...
class PiControlClient(
    ...
    fun login(providerId: String, authType: String, loginMethod: String? = null): String = command("login") {
        put("providerId", providerId)
        put("authType", authType)
        if (loginMethod != null) put("loginMethod", loginMethod)
    }
    ...
    ): String = command("configure_custom_provider") {
    ...
    fun removeCustomProvider(providerId: String): String = command("remove_custom_provider") {
        put("providerId", providerId)
    }
    ...
    private fun parseResponse(json: JSONObject): PiAuthEvent {
        ...
        if (!json.optBoolean("success")) {
            return PiAuthEvent.Error(command, json.optString("error", "Pi 操作失败"), requestId)
        }
        ...
        for (index in 0 until length()) {
            ...
            add(
                PiProvider(
                    ...
                    custom = value.optBoolean("custom"),
                )
            )
        }
    }
